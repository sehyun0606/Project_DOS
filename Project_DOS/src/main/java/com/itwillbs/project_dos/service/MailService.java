...
import com.itwillbs.project_dos.handler.GenerateRandomCode;
import com.itwillbs.project_dos.handler.MailClient;
...
public class MailService {
	...
	public MailAuthInfo sendAuthMail(MemberVO member) {
		...
		String auth_code = GenerateRandomCode.getRandomCode(8);
		System.out.println("생성된 인증 코드 : " + auth_code);
		// ===================================================================================
		// [ 인증 메일 발송 요청 ]
		// 인증 메일에 포함할 제목과 본문 생성
		String subject = "[DOS] 이메일 인증";
		
		// 인증코드만 발송하여 사용자가 인증 페이지에서 직접 코드값을 입력해야할 경우 사용
		String content = "<a>DOS 인증코드</a>" + "<br>" +  "인증코드 : " + auth_code;
